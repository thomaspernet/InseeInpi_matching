...
class aws_instantiate:
    def __init__(self, instance_aws, bucket = None, credential = None):
        """
    ...
    def client_boto_json(self):
        """
        Instanciates a S3 client with the credentials in json
        """
        with open(self.credential,'r') as f:
            load_cred=json.load(f)
            key=load_cred['aws_access_key_id']
            secret_=load_cred['aws_secret_access_key']
            
        client_=boto3.client('s3',
    ...
    def client_boto_csv(self):
        """
        Instanciates a S3 client with the credentials in csv
        """
    ...
    def gen_matching_stats_json(self, source, nature, origin, source_path,
     save_to_path, data_merged, save):
        """
        ...
        # save json to s3
        if save == 'yes':
            self.save_to_s3(file_name=json_filename,
            file_path=json_absolute_filepath)

        return json_

    def gen_full_matching(self, source, nature, origin, source_path,
     save_to_path, data_merged, save):
            """
    ...
    def gen_matching(self, source, nature, origin, save_to_path, data_merged,
     match_type, save):
            """
            ...
            gz_filename = "{}_{}_{}_{}.{}".format(source,nature,origin,
            path_suffix,'gz')
            save_to_full_filename = "{}/{}".format(save_to_full_path,
            gz_filename)
